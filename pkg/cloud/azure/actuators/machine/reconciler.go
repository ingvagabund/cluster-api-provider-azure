...
func (s *Reconciler) Update(ctx context.Context) error {
	...
	*/
	...
	// TODO(jchaloup): find a way how to propaget the hostname change in case
	...
	if vm.NetworkProfile != nil && vm.NetworkProfile.NetworkInterfaces != nil {
		for _, iface := range *vm.NetworkProfile.NetworkInterfaces {
			...
			if !ok {
				klog.Errorf("Unable to cast %q to network.Interface: %v", ifaceName, err)
			}
			...
			if niface.InterfacePropertiesFormat.IPConfigurations != nil {
				for _, ipConfig := range *niface.InterfacePropertiesFormat.IPConfigurations {
					...
					if ipConfig.PublicIPAddress != nil {
						if ipConfig.PublicIPAddress.ID != nil {
							...
							if !ok {
								klog.Errorf("public ip get returned invalid network interface")
								continue
							}
...
func (s *Reconciler) Delete(ctx context.Context) error {
	...
	// Create public IP resource if requested
	publicIPName := azure.GeneratePublicIPName(s.scope.Cluster.Name, fmt.Sprintf("%v-public-ip-address", s.scope.Machine.Name))
	err = s.publicIPSvc.Delete(ctx, &publicips.Spec{Name: publicIPName})
	if err != nil {
